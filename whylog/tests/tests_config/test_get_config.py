...
class TestBasic(TestCase):
    ...
    def find_config_in_parent_dir(cls, path):
        ...
        expected_path = os.path.join(path, YamlConfigFactory.WHYLOG_DIR)
    ...
    def test_find_config_in_home_directory(self):
        TestBasic.find_config_in_parent_dir(YamlConfigFactory.HOME_DIR)
    ...
    def tearDownClass(cls):
        ...
        for i in range(2):
            path, _ = os.path.split(path)
            TestBasic.remove_config_dir(path)
        TestBasic.remove_config_dir(YamlConfigFactory.HOME_DIR)
