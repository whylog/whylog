...
class TestBasic(TestCase):
    ...
    def test_concated_regex(self):
        ...
        assert sorted(concated._parsers_indexes.values()) == [(0, 3), (4, 4), (9, 4)]

        assert concated.get_extracted_regex_params("aaaaa") == {}
        assert concated.get_extracted_regex_params(content1)[parser1.name] == ["2015-12-03 12:08:09", "alfa36", "2"]
        assert len(concated.get_extracted_regex_params(content1)) == 1
        assert concated.get_extracted_regex_params(content2)[parser2.name] == ["2015-12-03 12:10:10", "alfa36", "alfa21", "2"]
        assert len(concated.get_extracted_regex_params(content2)) == 1
        assert concated.get_extracted_regex_params(content3)[parser3.name] == ["2015-12-03 12:11:00", "alfa21", "567.02", "101"]
        assert len(concated.get_extracted_regex_params(content3)) == 1
